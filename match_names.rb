...
Dir.foreach(folder) do |filename|
  ...
  markdown_match = content.match(/---\r?\n.+?\r?\n---(\r?\n)+(.+)$/m)
  markdown = markdown_match ? markdown_match[2] : nil
  
  api_title = data['title']
  h[api_title] = { api_id: api_id, markdown: markdown, **data }
end
...
titles.each do |title|
  ...
  content_intro = data['content_intro'] ? data['content_intro'].gsub("\n", '\n') : nil
  markdown = data[:markdown] ? data[:markdown].split(/\r?\n/).join('\n').gsub("\t", "  ") : nil
  puts "#{data['tagline']}#{data[:api_id]}\t#{title}\t#{content_intro}\t#{markdown}"
end
